...
# COMBINED FUNCTION 
# ------------------------------------------------------------------- #
def final_imputation(data, num_properties, impute_method = 'empty'):

    '''Impute missing values in data_x
  
    Args:
        - data: Dataset containing missing values stored in a DATAFRAME
    ...
    Returns:
        ...
        - mae_vector, rmse_vector: MAE and RMSE error metrics
    '''
    ...
    for i in range(0, y.shape[1]):
        ...
        if impute_method == 'final':
            ...
            #-------------------------------------------------------------------#
            # Full imputation (choose the method which gives lowest error)
            #-------------------------------------------------------------------#
            if KNN_tot_err < MF_tot_err: 
        ...
        # ------------------------------------------------------------------- #
            ...
            # Error calculation
            #-------------------------------------------------------------------#
            # Impute the known missing values, then calculate sample error metrics
            mf = MissForest()
            err_data_imputed = mf.fit_transform(err_data)

            # Convert the imputed dataset from an array back into a dataframe
            err_data_imputed = pd.DataFrame(err_data_imputed)  
            ...
            for k in range(NANcheck2.shape[0]):         # Iterate over rows
                for j in range(NANcheck2.shape[1]):     # Iterate over columns
                    val = NANcheck2.iat[k, j]           # If it had been a missing value here, then count twd error calculation
                    if val == True:
                        diff = err_data_full.iat[k, j] - err_data_imputed.iat[k, j]
                        abs_diff = abs(diff)
                        N = N + 1
                        rmse_summ = rmse_summ + diff**2
                        mae_summ = mae_summ + abs_diff
        ...
        # ------------------------------------------------------------------- #
            ...
            err_data_imputed = gain(err_data, gain_parameters)
            err_data_imputed = pd.DataFrame(err_data_imputed)
        ...
        # ------------------------------------------------------------------- #        
            ...
            err_data_imputed = imp.fit_transform(err_data)
            err_data_imputed = pd.DataFrame(err_data_imputed)
        ...
        # ------------------------------------------------------------------- #
            ...
            # Impute the known missing values, then calculate sample error metrics
            imputer = KNNImputer(n_neighbors=2)
            err_data_imputed = imputer.fit_transform(err_data)
            err_data_imputed = pd.DataFrame(err_data_imputed)  
        ...
        # ------------------------------------------------------------------- #
            ...
            err_data_imputed = pd.DataFrame(imputer.fit_transform(err_data))
    ...
    return data_imputed, mae_vector, rmse_vector
